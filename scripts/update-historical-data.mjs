...
import {
  fetchBasketballReferencePositions,
  findPositionRecord,
  POSITION_ELIGIBILITY_THRESHOLD,
} from './basketball-reference-positions.mjs';
...
const inferPosition = (p) =>
  n(p.REB) >= 8 || (n(p.BLK) >= 1.5 && n(p.AST) < 4) ? 'C' : n(p.AST) >= 4.5 ? 'G' : 'F';
...
async function getSeasonRows(season, attempt = 1) {
  ...
  Object.entries({ ...commonParams, MeasureType: 'Base', Season: season }).forEach(([key, value]) =>
  ...
  try {
    ...
    return set.rowSet.map((row) =>
      Object.fromEntries(set.headers.map((header, index) => [header, row[index]])),
    );
  } catch (error) {
...
function toPlayerSeason(p, season, positionMap) {
  ...
  const impactProxy = clamp(
    (points + rebounds * 0.7 + assists * 0.9 + steals * 1.8 + blocks * 1.6) / 4.2 - 5,
    -8,
    14,
  );
...
console.log(
  `Building Historic Mode data from ${seasonLabel(firstSeason)} through ${seasonLabel(lastSeason)}…`,
);
console.log(
  `Position eligibility uses Basketball-Reference estimates at a ${Math.round(POSITION_ELIGIBILITY_THRESHOLD * 100)}% threshold when available.`,
);
console.log(
  'The updater makes at most one NBA Stats and one Basketball-Reference request per season and can take several minutes.',
);
...
if (allPlayers.length < 100) {
  console.error(
    `Only ${allPlayers.length} historical player-seasons were downloaded. The existing historical database was not replaced.`,
  );
  process.exitCode = 1;
} else {
  allPlayers.sort((a, b) => a.season.localeCompare(b.season) || a.name.localeCompare(b.name));
  await writeFile(
    new URL('../src/data/historicalPlayers.json', import.meta.url),
