      ...
      <div className="glass mb-4 hidden rounded-2xl p-3 sm:mb-5 sm:block">
        <div className="grid grid-cols-2 gap-2 sm:gap-3 md:grid-cols-2 xl:grid-cols-[1.4fr_.7fr_.7fr_.8fr_auto]">
          ...
          <button
            onClick={onNewPool}
            disabled={mode === 'daily'}
            title={mode === 'daily' ? 'Daily pool is fixed for everyone' : 'Generate a new player pool'}
          ...
          {displayed.map((player, index) => {
            ...
            return (
              <motion.article
                ...
                transition={{delay:Math.min(index*.015,.25)}}
                key={player.id}
              ...
              >
                  ...
                  <div className="absolute bottom-3 left-4 right-4 flex items-end justify-between">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
                        {player.teamAbbreviation}{player.season ? ` · ${player.season}` : ''}
                      </p>
                      <h3 className="max-w-[155px] text-lg font-black leading-tight">{player.name}</h3>
                    </div>
                ...
                <div className="p-2.5 sm:p-4">
                  <div className="mb-3 grid grid-cols-5 sm:mb-4 divide-x divide-white/10 rounded-xl bg-black/20 py-3 text-center">
                    <div><p className="text-[9px] font-bold text-slate-500">PTS</p><p className="font-black">{player.points.toFixed(1)}</p></div>
                    <div><p className="text-[9px] font-bold text-slate-500">REB</p><p className="font-black">{player.rebounds.toFixed(1)}</p></div>
                    <div><p className="text-[9px] font-bold text-slate-500">AST</p><p className="font-black">{player.assists.toFixed(1)}</p></div>
                    <div><p className="text-[9px] font-bold text-slate-500">STL</p><p className="font-black">{player.steals.toFixed(1)}</p></div>
                    <div><p className="text-[9px] font-bold text-slate-500">BLK</p><p className="font-black">{player.blocks.toFixed(1)}</p></div>
                  </div>
                  ...
                  >
                    {active
                      ...
                      : <>Select Player<ChevronRight size={17}/></>}
                  </button>
                </div>
              </motion.article>
            );
