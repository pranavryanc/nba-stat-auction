...
function App() {
  ...
  return (
    <div className="app-shell min-h-screen overflow-x-hidden bg-[#050816] bg-[radial-gradient(circle_at_20%_0%,rgba(37,99,235,.22),transparent_28%),radial-gradient(circle_at_95%_10%,rgba(225,29,72,.16),transparent_24%)]">
      ...
      <AnimatePresence>{usernameEditorOpen && <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[90] grid place-items-center bg-black/75 p-4 backdrop-blur-sm" onClick={() => setUsernameEditorOpen(false)}><motion.div initial={{opacity:0,y:20,scale:.97}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:15,scale:.97}} onClick={event => event.stopPropagation()} className="w-full max-w-md rounded-3xl border border-white/10 bg-slate-950 p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.2em] text-blue-400">Profile</p><h2 className="mt-1 text-2xl font-black">Change username</h2><p className="mt-2 text-sm leading-6 text-slate-400">Your leaderboard name updates immediately. Usernames are unique.</p></div><button onClick={() => setUsernameEditorOpen(false)} className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white/5"><X size={18}/></button></div><input autoFocus value={usernameDraft} onChange={event => { setUsernameDraft(event.target.value); setUsernameError(''); }} onKeyDown={event => { if (event.key === 'Enter') saveUsername(); }} maxLength={20} className="mt-5 min-h-14 w-full rounded-2xl border border-white/10 bg-white/5 px-4 text-lg font-bold outline-none focus:border-blue-500/60"/><div className="mt-2 flex justify-between text-xs"><span className={usernameError ? 'text-rose-400' : 'text-slate-600'}>{usernameError || '3–20 characters · letters, numbers, _ or .'}</span><span className="text-slate-600">{usernameDraft.trim().length}/20</span></div><button onClick={saveUsername} disabled={usernameSaving || usernameDraft.trim() === username} className="mt-5 min-h-13 w-full rounded-xl bg-blue-500 py-3 font-black hover:bg-blue-400 disabled:opacity-40">{usernameSaving ? 'Saving…' : 'Save Username'}</button></motion.div></motion.div>}</AnimatePresence>
