...
const inferEligiblePositions = (p, primary) => {
  const positions = new Set([primary]);
  const assists = n(p.AST), rebounds = n(p.REB), blocks = n(p.BLK), threes = n(p.FG3A);
  if (primary === 'G' && (rebounds >= 4.5 || threes >= 3)) positions.add('F');
  if (primary === 'F' && assists >= 3.8) positions.add('G');
  if (primary === 'F' && (rebounds >= 7 || blocks >= 1.1)) positions.add('C');
  if (primary === 'C' && (threes >= 2 || assists >= 3.5 || rebounds < 8)) positions.add('F');
  return ['G', 'F', 'C'].filter(position => positions.has(position));
};
