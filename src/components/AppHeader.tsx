    ...
    <header className="safe-header sticky top-0 z-50 border-b border-white/10 bg-[#050816]/80 backdrop-blur-2xl">
      <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-2 px-3 py-3 sm:gap-4 sm:px-4 sm:py-4 md:px-7">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="hidden text-[10px] font-black uppercase tracking-[.25em] text-blue-400 sm:block">
        ...
        <div className="hidden items-center gap-2 lg:flex">
          <button
            onClick={onHome}
            className="rounded-xl px-3 py-2 text-sm font-semibold text-slate-300 hover:bg-white/5"
          >
          ...
          <button
            ...
            className="rounded-xl px-3 py-2 text-sm font-semibold text-slate-300 hover:bg-white/5"
          >
            <BarChart3 className="mr-2 inline" size={16} />
            Statistics
          </button>
          <button
            onClick={onProfile}
            className="rounded-xl px-3 py-2 text-sm font-semibold text-blue-300 hover:bg-white/5"
          >
          ...
          <button
            onClick={onSignOut}
            title={userEmail ?? ''}
            className="rounded-xl px-3 py-2 text-sm font-semibold text-slate-400 hover:bg-white/5 hover:text-white"
          >
        ...
        <div className="flex items-center gap-2 md:gap-4">
          <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-right">
            ...
            <motion.p
              ...
              initial={{ scale: 1.2 }}
              animate={{ scale: 1 }}
              className={`text-lg font-black ${remaining < 20 ? 'text-rose-400' : 'text-emerald-400'}`}
            >
              ${remaining}
            </motion.p>
          </div>
          <div className="min-w-[112px] rounded-xl border border-white/10 bg-white/5 px-2.5 py-2 text-right sm:min-w-[150px]">
            ...
            <div className="flex items-end justify-end gap-2">
              ...
              <p className="pb-0.5 text-[9px] font-black text-slate-400">
                G {guardCount}/2 · F {forwardCount}/2 · C {centerCount}/1
              </p>
            </div>
          </div>
        </div>
