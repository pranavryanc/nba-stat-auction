...
async function getRows(measureType) {
  ...
  Object.entries({ ...commonParams, MeasureType: measureType }).forEach(([k, v]) =>
    url.searchParams.set(k, v),
  );
  const response = await fetch(url, { headers });
  if (!response.ok)
    throw new Error(`NBA Stats request failed (${response.status}) for ${measureType}`);
  const json = await response.json();
  const set = json.resultSets?.[0];
  if (!set) throw new Error(`No ${measureType} result set returned.`);
  return set.rowSet.map((row) =>
    Object.fromEntries(set.headers.map((header, i) => [header, row[i]])),
  );
}
...
const inferPosition = (p) =>
  n(p.REB) >= 8 || (n(p.BLK) >= 1.4 && n(p.AST) < 4) ? 'C' : n(p.AST) >= 5 ? 'G' : 'F';
const inferEligiblePositions = (_p, primary) => [primary];

try {
  console.log(
    `Using a ${Math.round(POSITION_ELIGIBILITY_THRESHOLD * 100)}% positional-minutes threshold for eligibility.`,
  );
  ...
  try {
    ...
    console.warn(
      `Position estimates were unavailable; using the statistical fallback (${error instanceof Error ? error.message : error}).`,
    );
  }
  ...
  if (players.length < 80)
    throw new Error(
      `Only ${players.length} players were returned; refusing to overwrite the database.`,
    );
  await writeFile(
  ...
  console.log(
    `Updated src/data/players.json with ${players.length} players from the ${season} regular season.`,
  );
} catch (error) {
  console.error('\nCould not update player data.');
  console.error(error instanceof Error ? error.message : error);
  console.error(
    'Your existing players.json file was not changed. Try again on a normal internet connection.',
  );
  process.exitCode = 1;
}
