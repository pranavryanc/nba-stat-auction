      ...
      <div className="glass overflow-hidden rounded-3xl shadow-2xl">
        <div className="border-b border-white/10 bg-gradient-to-r from-blue-500/15 to-rose-500/10 p-5">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-[.2em] text-blue-400">Your roster</p>
              ...
              {selected.map(player => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-500">{positionText(player)} · ${player.price}</p>
                  </div>
                  <button
                    onClick={() => onTogglePlayer(player)}
...
function RosterRequirement({
  ...
  return (
    <div className={`rounded-xl border p-2 text-center ${complete ? 'border-emerald-400/30 bg-emerald-400/10' : 'border-white/10 bg-white/5'}`}>
      <p className="text-[9px] font-bold text-slate-500">{label}</p>
      <p className="font-black">{current}/{required}</p>
    </div>
  );
}
